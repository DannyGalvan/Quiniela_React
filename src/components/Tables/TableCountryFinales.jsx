...
import { apiCountryUsersFinales } from "../../api/apiUsers";

export const TableCountryFinales = ({ idCountry, width, title }) => {
  ...
  useEffect(() => {
    (async () => {
      const response = await apiCountryUsersFinales(idCountry);
      if (response?.data) {
        const posiciones = response.data.map((x, inx) => ({ ...x, pocicion: inx + 1 }));
        setUsers(posiciones);
      }
      setPending(false);
    })();
